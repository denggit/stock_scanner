...
class ExplosiveStockDataCollector:
    ...
    def _generate_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 5. 形态识别特征
        features['double_bottom'] = self._detect_double_bottom(df)
        features['flag_pattern'] = self._detect_flag_pattern(df)
        features['support_resistance'] = self._calculate_support_resistance(df)
