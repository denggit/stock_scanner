...
class CalIndicators:
    ...
    @staticmethod
    def ascending_channel(df: pd.DataFrame, config_path: Optional[str] = None, **params) -> Dict[str, Any]:
        """
        计算上升通道回归分析
        ...
        Args:
            ...
            **params: 自定义参数，可以覆盖配置文件中的参数
                - k: 通道宽度倍数
                - L_max: 最大窗口长度
                - delta_cut: 滑动窗口删除天数
                - pivot_m: 锚点检测参数
                - gain_trigger: 重锚涨幅触发阈值
                - beta_delta: 斜率变化阈值
                - break_days: 连续突破天数
                - reanchor_fail_max: 重锚失败最大次数
        ...
        try:
            # 初始化上升通道分析器
            analyzer = AscendingChannelRegression(config_path=config_path, **params)
    ...
    @staticmethod
    def ascending_channel_history(df: pd.DataFrame, min_window_size: int = 60,
                                  config_path: Optional[str] = None, **params) -> pd.DataFrame:
        """
        计算历史上升通道数据
        
        Args:
            df (pd.DataFrame): 价格数据，必须包含 trade_date, open, high, low, close, volume 列
            min_window_size (int): 最小窗口大小，默认60天
            config_path (Optional[str]): 配置文件路径
            **params: 自定义参数，可以覆盖配置文件中的参数
                - k: 通道宽度倍数
                - L_max: 最大窗口长度
        ...
        Returns:
            pd.DataFrame: 历史通道数据，每行对应一个时点的通道状态
        """
        if not ASCENDING_CHANNEL_AVAILABLE:
            raise ImportError("上升通道模块不可用")
        ...
        try:
            ...
            # 计算历史通道数据，传递自定义参数
            history_df = analyzer.fit_channel_history(df, min_window_size=min_window_size)

            return history_df

        except Exception as e:
            logging.error(f"历史上升通道计算失败: {e}")
            raise

    @staticmethod
    def ascending_channel_history_optimized(df: pd.DataFrame, min_window_size: int = 60,
                                            step_days: int = 1, config_path: Optional[str] = None,
                                            **params) -> pd.DataFrame:
        """
        优化的历史上升通道数据计算（按步长计算，提高效率）
        
        Args:
            df (pd.DataFrame): 价格数据，必须包含 trade_date, open, high, low, close, volume 列
            min_window_size (int): 最小窗口大小，默认60天
            step_days (int): 计算步长，默认1天，可以设为5天或10天提高效率
            config_path (Optional[str]): 配置文件路径
            **params: 自定义参数，可以覆盖配置文件中的参数
                - k: 通道宽度倍数
                - L_max: 最大窗口长度
        ...
        try:
            ...
            # 计算历史通道数据，传递自定义参数
            history_df = analyzer.fit_channel_history_optimized(df, min_window_size=min_window_size,
                                                                step_days=step_days)

            return history_df

        except Exception as e:
            logging.error(f"优化历史上升通道计算失败: {e}")
            raise

    @staticmethod
    def ascending_channel_history_incremental(history_df: pd.DataFrame, new_data: pd.DataFrame) -> pd.DataFrame:
        """
        增量更新历史通道数据（快捷调用）
        ...
        return analyzer.update_channel_history_incremental(history_df, new_data)

    @staticmethod
    def ascending_channel_history_with_state(history_df: pd.DataFrame,
                                             last_state,
                                             new_data: pd.DataFrame) -> pd.DataFrame:
        """
        基于状态对象的增量更新（快捷调用，推荐使用）
        ...
        analyzer = AscendingChannelRegression()
        return analyzer.update_channel_history_with_state(history_df, last_state, new_data)

    @staticmethod
    def ascending_channel_history_incremental_improved(history_df: pd.DataFrame,
                                                       original_df: pd.DataFrame,
                                                       new_data: pd.DataFrame) -> pd.DataFrame:
        """
        改进的增量更新历史通道数据（快捷调用，推荐使用）
        ...
        analyzer = AscendingChannelRegression()
        return analyzer.update_channel_history_incremental_improved(history_df, original_df, new_data)
